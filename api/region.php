...
if (isset($_GET['mpan'])) {
  ...
  if (!$payload) {
    $payload = goat_fetch("https://api.octopus.energy/v1/electricity-meter-points/$mpan/");
    if ($payload) goat_cache_write($key, $payload);
  }
...
if (isset($_GET['postcode'])) {
  ...
  $payload = goat_cache_read($key, $cacheTtl);
  if (!$payload) {
    $payload = goat_fetch("https://api.octopus.energy/v1/industry/grid-supply-points/?postcode=$pc");
    if ($payload) goat_cache_write($key, $payload);
  }
  $data = $payload ? json_decode($payload, true) : [];
