...
$start = $from ? new DateTime($from, $tz) : (new DateTime('now', $tz));
...
function http_get($url) { $ch=curl_init($url); curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>20,CURLOPT_USERAGENT=>'GOAT/1.0']); $res=curl_exec($ch); $code=curl_getinfo($ch,CURLINFO_HTTP_CODE); curl_close($ch); return ($code>=200&&$code<300)?$res:null; }
...
while($url){ $k='rates-'.sha1($url); $payload=cache_read($k,300); if(!$payload){ $payload=http_get($url); if($payload) cache_write($k,$payload);} if(!$payload) break; $json=json_decode($payload,true);
if (!empty($json['results'])) foreach($json['results'] as $r){ $inc = isset($r['value_inc_vat']) ? $r['value_inc_vat'] : (isset($r['value_exc_vat']) ? round($r['value_exc_vat']*1.05,3) : null); if($inc===null) continue;
  $rates[]=['start'=>$r['valid_from'],'end'=>$r['valid_to'],'price_inc_vat_p_per_kwh'=>$inc]; }
$url=$json['next']??null; }
usort($rates,function($a,$b){return strcmp($a['start'],$b['start']);}); echo json_encode($rates, JSON_UNESCAPED_SLASHES);
